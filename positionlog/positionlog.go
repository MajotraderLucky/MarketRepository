...
func WritePositionsToFile(autoGeneratedPos AutoGeneratedPos) error {
	...
	if positionBTCindex == -1 {
		return errors.New("no BTCUSDT position found")
	}
	...
	// Encoding the data in JSON format using MarshalIndent for prettier output.
...
func GetPositionData(autoGeneratedPos AutoGeneratedPos) (string, string, error) {
	for _, p := range autoGeneratedPos.Positions {
		if p.Symbol == "BTCUSDT" {
			return p.PositionAmt, p.EntryPrice, nil
		}
	}
	return "", "", errors.New("No BTCUSDT position found")
}
